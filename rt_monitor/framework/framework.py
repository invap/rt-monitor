...
class Framework:
    ...
    def framework_from_file(spec_file):
        ...
        try:
            ...
            logger.error(
                f"Permissions error opening file [ {spec_complete_filename} ]."
            )
            raise FrameworkError()
    ...
    def framework_from_dict(framework_dict, working_path):
        ...
        if "components" in framework_dict:
            ...
            for variable in process.variables():
                if process.variables()[variable][0] == "Component" and not any(
                    [
                        variable in components[component].state()
                        for component in components
                    ]
                ):
                    logger.error(
                        f"Variable [ {variable} ] not declared in any component."
                    )
                    raise FrameworkError()
        else:
            components = {}
            if any(
                [
                    process.variables()[variable][0] == "Component"
                    for variable in process.variables()
                ]
            ):
                logger.error(
                    f"Process contains component variables but no components were defined."
                )
                raise FrameworkError()
    ...
    def _parse_process(process_dict, working_path):
        ...
        factory = {"graph": GraphProcess, "regex": RegExProcess}
    ...
    def _parse_components(component_dict):
        ...
        for component in component_dict:
            ...
            try:
                ...
                logger.error(
                    f"Component file attribute missing in component [ {device_name} ] specification."
                )
            ...
            try:
                ...
                logger.critical(
                    f"Loading component [ {component_name} ] form [ {component_path} ] error: {str(e)}."
                )
                raise ComponentsSpecificationError()
            if spec is None:
                logger.error(
            ...
            try:
                ...
                logger.error(
                    f"Loader attribute for component [ {device_name} ] not found."
                )
                raise ComponentsSpecificationError()
            except TypeError:
            ...
            try:
                ...
                logger.error(
                    f"The module for component [ {device_name} ] contains invalid python syntax."
                )
                raise ComponentsSpecificationError()
                ...
                logger.error(
                    f"Permission error opening file for component [ {device_name} ]."
                )
                raise ComponentsSpecificationError()
            try:
                ...
                logger.error(
                    f"Component name attribute missing in component [ {device_name} ] specification."
                )
                raise ComponentsSpecificationError()
